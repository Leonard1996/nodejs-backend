...
export class ProductService {
  ...
  static getLatest = () => {
    return getManager().query(
      `select * from products where job_id in (Select max(jobs.id) from jobs where status = 'SUCCESS')`
    );
  };
  ...
  static async update(product) {
    ...
    product = productRepository.merge(oldProduct, {
      raw: JSON.stringify(product),
      offer: JSON.stringify(product.offers),
      brand: JSON.stringify(product.brand),
      name: product.name,
